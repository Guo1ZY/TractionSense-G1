#!/usr/bin/env bash
# Isolated low-friction delayed-sensor recovery from model_7890.
set -euo pipefail
...
TASK="Unitree-G1-29dof-Velocity-Foot-TractionTeacher-Robust-LowMu-Recovery"
...
SEED="${SEED:-86}"
RUN_NAME="${RUN_NAME:-low_mu_delay_recovery_from_7890}"
EXTRA=()

while [[ $# -gt 0 ]]; do
  case "$1" in
    --smoke) NUM_ENVS=128; MAX_ITERS=2; RUN_NAME="low_mu_recovery_smoke"; shift ;;
