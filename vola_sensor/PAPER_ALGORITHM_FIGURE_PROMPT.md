# 论文算法图 Prompt（完整版）：1864 观察 + 完整策略架构 + 教师–学生训练结构

> 本版本包含全部细节：观察张量展开、(a)/(b)/(c) 三面板、冻结教师内部、
> 门控残差支路、以及训练侧教师–学生（anchor / 蒸馏 / stage-BCE）。
> 所有数字已于 2026-08-13 与代码逐项核对（25/25 通过，见文末核对清单）。
> 若只要极简版，见精简版分支的旧版本（git/备份）。

配套文件：
- matplotlib 精确版（数字不会错，推荐投稿终稿）：`vis/paper_figs/fig_policy_arch_style_test.pdf`
- AI 生图版（本文件 Prompt）：生成后必须用矢量软件补字（AI 会拼错密集数字标签）

## 与代码核对的数字清单（25/25 ✓）

| 项 | 值 | 来源 |
|---|---|---|
| 观察总维度 | 1864 = 480 + 1350 + 30 + 2 + 2 | `frozen_speedboost_teacher.py` |
| 本体每帧 | 96 = ω3 + g3 + cmd3 + q29 + q̇29 + a29，5 因果帧 | obs 项维度审计 |
| Hall 历史 | 1350 = 15帧 × 2脚 × 15点 × 3轴 | `MAGNETIC_DIM` |
| 采样周期 | 30 = 15帧 × 2脚 | `PERIOD_DIM` |
| 编码器 | 每点 MLP 3→16→16；每帧 MLP 241(=15×16+1)→64→32；Conv1d k=3 ×3；每脚 latent 32 | `SharedFootEncoder` |
| 教师融合维度 | 548 = 480 + 32×2 + 4 | `FUSED_DIM` |
| 教师子策略 | safe / fast(带 μ̂ 头) / stable，各 548→512→256→128→29 | `SharedMagneticPolicy` |
| μ̂ 牵引门 | σ((μ̂−0.65)·10)，μ̂∈(0,1.30] | `SpeedBoostTeacherConfig` |
| 置信度 | calib·evidence·valid·fresh | `confidence()` |
| boost | 1.12×conf×traction×σ((vx−0.70)·15)，lerp 混合 | `boost_gate()` |
| gate 头 | 548→128→32→1 → sigmoid → 校准 σ(2.75·logit−3.2) → ×min(valid_L,valid_R) | `fastbase_capture_residual.py` + agent.yaml |
| residual 头 | 548→256→128→29 → δ = 0.55·tanh(·) | 同上 |
| 合成 | a = a_base + g·δ，clamp ±3，29 关节位置动作 | `forward()` |
| 训练：anchor | 复合输出锚定 speedboost112 教师（HIGH 段），cap 0.3 | agent.yaml |
| 训练：蒸馏 | Stage7 低摩擦专家（0.16 m/s 反事实指令）监督 residual，`supervised_only` | agent.yaml |
| 训练：gate | 仅 stage-BCE 监督（梯度隔离），high-end 权重 4.0 | agent.yaml |
| 信号边界 | 力/接触/摩擦/滑移不进 Actor（ABI 审计禁词） | 评估器审计 |

## 可直接用于 Gemini 的完整 Prompt
...
- 数字标签（1864/480/1350/548/29/0.55/2.75/−3.2…）AI 大概率拼错，必须矢量软件补字；
- 投稿终稿推荐用 matplotlib 版（数字精确、PDF 矢量）：`vis/paper_figs/fig_policy_arch_style_test.pdf`；
- 若 AI 生图失败或排版混乱，退回"精简版"（4 块 + 训练侧 3 箭头），核心语义不丢。
