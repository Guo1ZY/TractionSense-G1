"""Legacy F0T1 Hall-to-force experiment and shared BLE receiver utilities.

The executable path in this module is disabled: ``FootSensor15`` does not
measure normal or tangential force.  ``magnetic_bridge.py`` reuses only the raw
BLE receiver/state/logger helpers and publishes F0M1 magnetic observations.
"""
...
class HallSample:
    """One physical foot sample stamped by the common host monotonic clock."""
...
class FootState:
    ...
    samples: deque[HallSample] = field(
        ...
        row.update(
            {
                ...
                for name, value in zip(MAG_COLUMNS, state.magnetic_xyz.reshape(-1))
            }
        )
...
class SensorPipeline:
    ...
    def reset(self) -> None:
        ...
        self.state.last_wall_ns = 0
        self.state.last_monotonic = 0.0
    ...
    def receive(self, data: bytes) -> None:
        try:
            ...
            if self.state.last_monotonic > 0.0:
                period = frame.received_monotonic - self.state.last_monotonic
                if 0.001 <= period <= 0.25:
                    self.state.sample_period_s = period
            ...
            self.state.samples.append(
                HallSample(
                    received_wall_ns=frame.received_wall_ns,
                    received_monotonic=frame.received_monotonic,
                    source_sequence=frame.source_sequence,
                    sample_period_s=self.state.sample_period_s,
                    temperature_x10=frame.temperature_x10.copy(),
                    magnetic_xyz=self._filtered.copy(),
                )
            )
            self.state.frames += 1
...
def configured_adapters(config: dict[str, Any]) -> dict[str, str]:
    """Return an explicit one-controller-per-foot assignment when configured."""
    adapters = {
        side: str(config[side].get("adapter", "")).strip()
        for side in ("left", "right")
    }
    if any(adapters.values()) and not all(adapters.values()):
        raise ValueError("left.adapter and right.adapter must be configured together")
    if adapters["left"] and adapters["left"] == adapters["right"]:
        raise ValueError("left.adapter and right.adapter must be different")
    return adapters
        ...
        try:
            ...
            if state.adapter:
                if "bluez" in inspect.signature(BleakClient).parameters:
                    client_options["bluez"] = {"adapter": state.adapter}
                else:
                    client_options["adapter"] = state.adapter
...
async def run_hardware(args: argparse.Namespace, config: dict[str, Any]) -> int:
    ...
    states = {
        side: FootState(side=side, address=addresses[side])
        for side in ("left", "right")
    }
...
def main() -> int:
    print(
        "[ERROR] F0T1 Hall-to-force publishing is disabled: FootSensor15 "
        "provides Hall counts, not normal/tangential force. Use "
        "run_magnetic_bridge.py (F0M1) or record_raw_hall.py."
    )
    return 2
