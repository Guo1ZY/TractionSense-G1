...
public:
    ...
    :ActionTerm(cfg, env)
    {
        ...
        // Older exported policies do not contain raw_clip at all.  A missing
        // yaml-cpp node is Undefined (not Null), so checking only IsNull()
        // attempts a vector conversion and aborts while the FSM constructs
        // those legacy policy states.
        const YAML::Node raw_clip = cfg["raw_clip"];
        if(raw_clip && !raw_clip.IsNull()) {
            _raw_clip = raw_clip.as<std::vector<float>>();
            if(_raw_clip.size() != 2 || _raw_clip[0] > _raw_clip[1]) {
                throw std::runtime_error("raw_clip must be [minimum, maximum]");
            }
        }
    }
    ...
    virtual void process_actions(std::vector<float> actions)
    {
        ...
        {
            if(!_scale.empty()) {
                const float raw = _raw_clip.empty()
                    ? _raw_actions[i]
                    : std::clamp(_raw_actions[i], _raw_clip[0], _raw_clip[1]);
                _processed_actions[i] = raw * _scale[i];
            } else {
                _processed_actions[i] = _raw_clip.empty()
                    ? _raw_actions[i]
                    : std::clamp(_raw_actions[i], _raw_clip[0], _raw_clip[1]);
            }
            if(!_offset.empty()) {
